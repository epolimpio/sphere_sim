...
!    triangulation unless IER /= 0, in which case NT = 0.  NT = 2N - NB- 2, 
...
!    of its index.
!
!    Output, integer ( kind = 4 ) LPTR(6*N-12), pointers (LIST indexes) in 
!    one-to-one correspondence with the elements of LIST.  LIST(LPTR(I)) 
!    indexes the node which follows LIST(I) in cyclical counterclockwise order
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) N, the number of nodes in the grid.  N = NX*NY
!    for some NY >= 2.
!
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) LUN, the logical unit number in the range 
...
!
!    Input, integer ( kind = 4 ) LCC(NCC) (or dummy parameter if NCC = 0) 
!    containing the index of the first node of constraint I in LCC(I).  For 
!    I = 1 to NCC, LCC(I+1)-LCC(I) >= 3, where LCC(NCC+1) = N+1.
!
!    Input, integer ( kind = 4 ) N, the number of nodes in the triangulation.  
!    3 <= N.
!
